 ...
 * 
 * Edited functions by: Venetia Furtado
 * *** Original Library does not use these functions***
 * initAcars_serial()
 * blk_thread_serial()
********************************************************************************
/*
...
/*This code is edited for blocked reading; original function is void blk_thread() above*/
void blk_thread_serial()
{
		msgblk_t *blk;
		int i, pn;
		unsigned short crc;
...
/*This code is edited for blocked reading; original function is initAcars() below*/
int initAcars_serial(channel_t * ch)
{
	if(ch->chn==0) {
        	blkq_e=blkq_s=NULL;	
			acars_shutdown = 0;
...
void decodeAcars_serial(channel_t * ch)
{
	unsigned char r = ch->outbits;
	//printf("decodeAcars_serial: %d\n", ch->Acarsstate);
