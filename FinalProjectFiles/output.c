...
{
	oooi_t oooi;
	syslog(LOG_INFO, "ACARS printmsg called");
	//printf("ACARS printmsg called\n");
	...
	if(msg->bid) 
	{
		syslog(LOG_INFO, "ACARS: Aircraft reg %s", msg->addr);
		printf("ACARS: Aircraft reg %s\n", msg->addr);
		return;
	}
...
static void printmonitor(acarsmsg_t * msg, int chn, struct timeval tv)
{
	...
	fprintf(fdout,"             Acarsdec monitor "); printtime(tv);
	...
	while(fl) {
		...
		fprintf(fdout," %-8s %-7s %3d ", fl->addr, fl->fid,fl->nbm);
		for(i=0;i<nbch;i++) fprintf(fdout,"%c",(fl->chm&(1<<i))?'x':'.');
		for(;i<MAXNBCHANNELS;i++) fprintf(fdout," ");
		fprintf(fdout," "); printtime(fl->ts);
        	if(fl->oooi.sa[0]) fprintf(fdout," %4s ",fl->oooi.sa); else fprintf(fdout,"      ");
		if(fl->oooi.da[0]) fprintf(fdout," %4s ",fl->oooi.da); else fprintf(fdout,"      ");
        	if(fl->oooi.eta[0]) fprintf(fdout," %4s ",fl->oooi.eta); else fprintf(fdout,"      ");
		fprintf(fdout,"\n");
